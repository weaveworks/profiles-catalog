...
create-cluster:
	@if [ ${INFRASTRUCTURE} = "kind" ]; then\
		...
		gcloud container clusters create ${GKE_CLUSTER_NAME} --region ${GCP_REGION}  --enable-autoscaling --max-nodes ${GCP_NUM_NODES} --min-nodes ${GCP_NUM_NODES} --project ${GCP_PROJECT_NAME}
	fi
